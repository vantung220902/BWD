            ...
            <div class="row pb-0">
                <div class="col-lg-8 ">
                    ...
                    <table class="table table-hover table-striped text-center">
                        ...
                        <%
                            List<Product> lsCartPro = (ArrayList<Product>) request.getAttribute("listCartPro");
                            
                        %>
                        <tbody>
                            <%
                                for (int i = 0; i < lsCartPro.size(); i++) {
                                    Product pro = lsCartPro.get(i);
                                    if(pro.getColor().equals("#ffffff")){
                                        pro.setColor("none");
                                    }
                            %>
                            <tr>
                                <td class="cart-product-img" style="position: relative">
                                    <div class="layout_img" style="background:<%=pro.getColor()%> ">
                                        
                                    </div>
                                    <img src="<%=pro.getImgFont()%>" alt="img">
                                </td>
                                <td><%=pro.getName()%></td>
                                ...
                                <td>
                                    ...
                                    <a href="SubServlet?idCart=<%=pro.getId()%>"><button style="font-size: 20px" class="btnSub btn btn-light">-</button></a>
                                    <strong style="padding:0 10px;"><%=pro.getQuantity()%></strong>
                                    <c:if test="${check}">
                                        <a href="CartServlet?idCart=<%=pro.getId()%>&size=<%=pro.getQuantity()%>"><button style="font-size: 20px" class="btnAdd btn btn-light">+</button></a>
                                    </c:if>
                                    <h6>${tb}</h6>
                                </td>

                                <td>
                                    <a href="RemoProServlet?idCart=<%=pro.getId()%>">
                    ...
                    <div class="col-md-4 col-sm-4 ip-back">
                        <a href="ProductServlet?action=SearchProductCT&id=${sessionScope.idCTSession}&indexpage=${sessionScope.indexCurrentSession}"><input type="submit" class="back-shopping" value="Continue Shopping"></a>
                    </div>
